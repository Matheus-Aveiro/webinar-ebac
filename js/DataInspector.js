...
class Inspector {
  ...
  constructor(divid, diagram, options) {
    ...
    const mainDiv =
      divid instanceof HTMLDivElement ? divid : document.getElementById(divid);
  ...
  set propertyModified(val) {
    if (val !== this._propertyModified) {
      if (val !== null && typeof val !== "function")
        throw new Error(
          "Inspector.propertyModified must be a function or null"
        );
  ...
  inspectObject(object) {
    ...
    if (inspectedObjects) {
      ...
      let data =
        inspectedObject instanceof go.Part
      ...
      while (
        it.next() &&
        (this._showLimit < 1 || nodecount <= this._showLimit)
      ) {
          ...
          while (addIt.next()) {
            if (properties.has(addIt.key)) {
              ...
              if (
                ...
                newVal =
                  shared.get(addIt.key) + "|" + properties.get(addIt.key);
          ...
          while (addIt.next()) {
            if (all.has(addIt.key)) {
              if (
                ...
                // for non-string properties i.e color
                const newVal =
                  all.get(addIt.key) + "|" + properties.get(addIt.key);
                all.set(addIt.key, newVal);
              }
            } else {
              let newVal = "";
